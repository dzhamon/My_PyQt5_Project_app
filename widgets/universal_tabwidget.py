...
class UniversalTabWidget(QWidget):
	...
	def init_list_widgets(self, layout):
		...
		for name in self.params:
			...
			if self.use_timer:
				# Используем таймер для отложенной фильтрации
				filter_timer = QTimer(self)
				filter_timer.setSingleShot(True)
				filter_timer.timeout.connect(lambda: self.filter_listbox(search_entry.text(), list_widget))
				search_entry.textChanged.connect(filter_timer.start(300))
			else:
				search_entry.textChanged.connect(lambda text: self.filter_listbox(text, list_widget))
