...
class Tab6Widget(QWidget):
	...
	def __init__(self, params_for_tab6):
		...
		self.list_widgets = {}
		self.filtered_df = pd.DataFrame  # пустой датафрейм для хранения отфильтрованных данных
	...
	def update_data(self, filtered_df):
		...
		if filtered_df.empty:
			...
			return
		self.filtered_df = filtered_df  # сохраняем отфильрованный DataFrame
		...
		for col, (list_widget, search_entry) in self.list_widgets.items():
			...
			# Стбрасываем full_list для обновленных данных
